...
date: 2021-08-28 12:52:19
permalink: /pages/60f05b/
---
# JavaScript (1)

## 0 前言

* 本文为整合本人之前看视频的JavaScript笔记和《JavaScript高级程序设计（第三版）》的笔记。

* 适合有一定的js基础上复习使用。

* 本文会提及少量es6内容，但不做深入了解。深入了解请看文章es6
...
所谓标识符，就是变量、函数、属性或函数参数的名称。**ECMAScript 标识符使用驼峰大小写形式**，即第一个单词的首字母小写，后面每个单词的首字母大写。
...
ECMAScript 变量是松散类型的，意思是变量可以用于保存任何类型的数据。每个变量只不过是一个用于保存任意值的命名占位符。变量声明在 es5 使用 var （已废弃）,在 es6 使用 let 和 const。
...
ECMA-262 描述了一组保留的关键字，这些关键字有特殊用途，比如表示控制语句的开始和结束，或者执行特定的操作。按照规定，**保留的关键字不能用作标识符或属性名**。ECMA-262 第6 版规定的所有关键字如下：
...
> 看本章之前建议先看运算符章节。放在变量中是因为本文不是完全按照学习顺序划分章节，而是通过功能划分。
...
2. 如果对**非 String**使用 parseInt()或 parseFloat()，它会**先将其转换为 String** 然后再操作。⭐️
...
2. ⭐️我们可以对一个其他的数据类型使用`+`，来将其转换为number。（非常重要）
...
### 5.6 ⭐️break 和 continue

> 非常重要

**break**

- break 可以用来退出 switch 语句或退出**整个**循环语句（循环语句包括 for 循环、while 循环。不包括 if。单独的if 语句里不能用 break 和 continue，否则会报错）。（重要）
- break 会立即终止离它**最近**的那个循环语句。（重要）
- 可以为循环语句创建一个 label，来标识当前的循环（格式：label:循环语句）。使用 break 语句时，可以在 break 后跟着一个 label，这样 break 将会结束指定的循环，而不是最近的。(不重要)

**continue**

- continue 可以用来跳过**当次**循环，继续下一次循环。（重要）
- 同样，continue 默认只会离他**最近**的循环起作用。（重要）
- 同样，如果需要跳过指定的当次循环，可以使用 label 标签。(不重要)